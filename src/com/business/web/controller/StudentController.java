...
public class StudentController extends BaseController{
	...
	@RequestMapping(value="/super/student/step1.html",method=RequestMethod.GET)
	public String studentStep1(HttpServletRequest request,HttpServletResponse response){
		List<School> sclList = this.getBaseService().loadAll(School.class);
		request.getSession().setAttribute("SCHOOLS", sclList);
		return "import_student_step1";
	}
	
	@RequestMapping(value="/super/student/{id}",method=RequestMethod.GET)
	public void fileDownLoad(@PathVariable String id,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		String fileName = "student-data.xls";
        ...
        try {
        	...
        	ExcelUtil.createStudentTemplate(grdList).write(os);
        } catch (IOException e) {
        	e.printStackTrace();
        }
	...
	@RequestMapping(value="/super/student/step2.html",method=RequestMethod.POST)
	public String studentStep2(DefaultMultipartHttpServletRequest request){
		
		String rtUrl = "import_student_step2";
		String savePath = request.getSession().getServletContext().getRealPath("/WEB-INF/upload");
		String tempPath = request.getSession().getServletContext().getRealPath("/WEB-INF/temp");
		List<StudentExcelVO> stList = null;
		...
		try{
			...
			for(Entry e: map.entrySet()){
				...
				if(doSave(f.getFileItem(),savePath)){
					...
					stList = ExcelUtil.readStudentFile(savePath);
				}else{
					rtUrl= "import_student_step1";
				}
			}
		...
		request.getSession().setAttribute("STUDENTDATA", stList);
		return rtUrl;
		
	}
	
	@RequestMapping(value="/super/student/data",method=RequestMethod.GET)
	@ResponseBody List<StudentExcelVO> getData(HttpServletRequest request){
		List<StudentExcelVO> stData = (List<StudentExcelVO>)request.getSession().getAttribute("STUDENTDATA");
		verifyData(stData);
		return stData;
	}
	
	@RequestMapping(value="/super/student/data/update",method=RequestMethod.POST)
	@ResponseBody void updateData(HttpServletRequest request) throws Exception{
		List<StudentExcelVO> stData = (List<StudentExcelVO>)request.getSession().getAttribute("STUDENTDATA");
		int index = Integer.valueOf(request.getParameter("index"));
		String field = request.getParameter("field");
		String value = request.getParameter("value");
		
		StudentExcelVO st = stData.get(index);
		String methodName = "set"+field.substring(0, 1).toUpperCase()+field.substring(1);
		Method setMethod = st.getClass().getDeclaredMethod(methodName, String.class);
		setMethod.invoke(st, value);
	}
	
	@RequestMapping(value="/super/student/step3.html",method=RequestMethod.POST)
	public String studentStep3(HttpServletRequest request){
		String url = "import_student_step3";
		List<StudentExcelVO> stData = (List<StudentExcelVO>)request.getSession().getAttribute("STUDENTDATA");
		if(!verifyData(stData)){
			url = "import_student_step2";
	...
	@RequestMapping(value="/super/student/step4.html",method=RequestMethod.POST)
	public String studentStep4(HttpServletRequest request){
		String url = "import_student_step3";
		List<StudentExcelVO> stData = (List<StudentExcelVO>)request.getSession().getAttribute("STUDENTDATA");
		if(persisteManger.saveStudentInfo(stData)){
			url = "import_student_step1";
			request.getSession().removeAttribute("ERROR");
		}else{
			request.getSession().setAttribute("ERROR", "学生数据保存失败!");
		}
...
//		 String dir = savePath + "\\" + dir1 + "\\" + dir2;  //upload\2\3  upload\3\5
	...
	private boolean verifyData(List<StudentExcelVO> stData){
		boolean isSuccess = true;
		//verify data
		for(StudentExcelVO t:stData){
			...
			//3. verify student code format
			if("".equals(t.getStuCode())||t.getStuCode().length()>10){
				t.setIsValid("No");
				isSuccess = false;
				continue;
			}
			//4. verify student name
			if("".equals(t.getStuName())||t.getStuName().length()>30){
				t.setIsValid("No");
				isSuccess = false;
				continue;
			}
			//5. verify student gender
			//6. verify student age
			//7. verify student phone
			//8. verify student Email address
			t.setIsValid("Yes");
		}
		return isSuccess;
	}
