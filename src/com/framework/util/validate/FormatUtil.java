 ...
 * 
 */ 
package com.framework.util.validate;
...
public class FormatUtil {
	...
	/** 数据库所需的转换：true--> 7 false-->8  */
	public static Long toLong4Dict(boolean status){
		if(status) return FrameConst.YES;return FrameConst.NO;
	}
	public static boolean toBooleanSeven2Yes(Long status){
		if(status == FrameConst.YES) return true;return false;
	} 
	/** 数据库所需的转换："true"--> 7 "false"-->8  */
	public static Long toLong4Dict(String status){
		if(ValidateUtil.isBlank(status)) return FrameConst.NO;		
		if("TRUE".equals(status.toUpperCase().trim())) return FrameConst.YES;
		return FrameConst.NO;
	} 
